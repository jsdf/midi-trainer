...
export default function MidiDeviceSelector<
  TMidiPort extends WebMidi.MIDIInput | WebMidi.MIDIOutput,
>({
  type,
  onChange,
  selectedPort,
  ...
  React.useEffect(() => {
    ...
    navigator.requestMIDIAccess().then((access) => {
      const ports = getPorts(access, type);
      setAvailablePorts(ports as TMidiPort[]);
      if (initialPortID != null) {
        const initial = (ports as TMidiPort[]).find(
          (port) => port.id === initialPortID
        );
        if (initial) {
          onChange(initial);
        }
      ...
      access.onstatechange = function (e) {
        setAvailablePorts(getPorts(access, type) as TMidiPort[]);
      };
    });
  }, []);
